...
void RunProgram(ofstream & out, vector<long long> & v, vector<vector<char>> & matrix)
{
  ...
  while (true)
  {
    ...
    if (opcode == 99)
    {
      ...
      // Part 2
      auto [minXIt, maxXIt] = minmax_element(points.begin(), points.end(), [](const Point & p1, const Point & p2) { return p1.x < p2.x; });
      auto [minYIt, maxYIt] = minmax_element(points.begin(), points.end(), [](const Point & p1, const Point & p2) { return p1.y < p2.y; });
...
int main()
{
  ...
  vector<vector<char>> matrix(originX * 2, vector<char>(originY * 2, '.'));
